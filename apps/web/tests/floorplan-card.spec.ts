...
// floorplan 카드 행 (P3-2) — API mock(키리스). 객관 feature(bay·향·판상/타워) 표시 · null-tolerant
// (일부만/미조사) · 출처 딥링크 · gym/pet/review 공존(회귀 0) · 콘솔에러 0. **표시 전용**(랭킹 무관).
...
const base = {
  ...
  pet: {
    pet_allowed: "yes", confidence: 0.8, evidence: "허용", caveats: [],
...
const CANDIDATES = [
  ...
  },
  {
    ...
    complex_id: "A2",
    name: "부분추출단지",
    // null-tolerant: structure만 읽힘(bay·향 null) → 있는 것만 표시
...
test("floorplan row: features / null-tolerant partial / none / link / coexist", async ({ page }) => {
  ...
  // A1 — 완전: 3bay·남향·판상형 + evidence + http 링크 + gym/pet 공존(회귀 0).
