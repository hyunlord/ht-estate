...
def geocode_fingerprint(conn: sqlite3.Connection) -> tuple[str, int]:
    ...
    """
    rows = conn.execute(
        "SELECT complex_id, lat, lng FROM complex WHERE property_type='apartment' "
        "ORDER BY complex_id"
    ).fetchall()
