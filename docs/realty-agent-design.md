...
complex (
  ...
  has_gym           BOOLEAN,            -- 파생: 헬스/피트니스/운동시설 키워드 파싱
...
2. **단지정보 적재**: 단지코드별 K-apt 기본정보 → `amenities_raw`에서 `has_gym` 파싱, `parking_ratio = parking_total / household_count` 계산, 지하주차·사용승인일·복도유형 매핑
...
```json
{
  "hard": { "has_gym": true, "approval_year": [2010, 2022],
            "net_area": { "around": 84 }, "parking_underground": true,
            "parking_ratio": { "gte": 1.3 } },
  "soft": { "pet_allowed": "required", "floorplan": "good" }
}
```
...
- **Phase 0 — Tier 1만으로 네이버 초월**
  ...
  → 이 시점에 이미 "헬스장+사용승인일+전용+지하주차+세대당주차" 필터가 됨(네이버 불가).
