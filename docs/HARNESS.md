...
- id: feature = `T0-x`, 잡일(S) = `C<n>`, 스파이크/프로브(§6.4) = `R<n>` (번호 충돌 방지).
...
조사·리서치·프로브용 티켓. 산출물 = **발견 리포트 + 권고**(배포 코드 아님).
- **프로토콜**: 간단 PLAN만. §7 DEBATE/CHALLENGE rigor·§8.2 100점 루브릭 **미적용**.
- **게이트**: app 코드 무변경이면 §8.1 객관 게이트 N/A(프로브 스크립트·docs만 — `make gate` 회귀 0만 확인). 코드가 바뀌면 바뀐 부분만 §8.1.
- **Web 검증**: 코드 재채점이 아니라 **근거·권고의 타당성**을 평가한다.
- id: `R<n>`. — 사례: R1 헬스장 신호 프로브(K-apt 부대시설에 헬스장 0/17 → Tier-2 enrichment 권고).
...
- **R1 — setup/scaffold 채점 carve-out**: 셋업·스캐폴드·설정·인프라 성격의 M·L 티켓(런타임 시연 대상 자체가 없는 것)은 완결성·안정성을 "value를 demonstrate"가 아니라 **"스펙 충족 + 하류 안 막음"**으로 채점한다. 스캐폴드에 demonstrate를 강요하면 YAGNI 패딩을 유발하기 때문. ([S] triage는 루브릭 자체를 생략하지만, R1은 **루브릭을 적용하되 채점 렌즈만 바꾸는** setup 성격의 M·L을 커버한다.) — 사례: T0-0 self 93이나 carve-out 렌즈로는 ~99.

### 8.3 Web 독립 검증
Code의 스코어카드는 참고용. Web는 실제 diff/raw/스크린샷으로 8.1·8.2를 **다시** 돌려 자체 판정한다. 불일치하면 **Web 판정이 이긴다.**

### 8.4 외부 선결조건 deferral — 조건부 통과 (R3)

티켓의 완전 검증이 **사람 액션이 필요한 외부 선결조건**(신청·심사 뒤의 데이터/키/접근)에 막혀 있고, **의뢰서가 그 검증을 명시적으로 deferral**한 경우:
- **채점**: in-scope 산출물만 기준으로 매긴다. deferral된 검증으로 완결성·안정성을 깎지 않는다.
- **조건부 통과**: in-scope가 기준(≥95+하한, 또는 setup이면 §8.2 R1 렌즈) 충족이면 머지 가능. 단 선결조건 충족 후 후속 티켓이 deferral 검증을 수행하기 전까지 **"완결" 아님** — 조건부 상태로 추적한다.
- **풀 통과**: 후속(예 `<ticket>-live`)이 선결조건 충족 뒤 deferral된 검증을 수행 → 거기서 **이전 deferral 차원을 포함해 풀 통과(≥95).**
- **가드**: 의뢰서에 **명시적으로** deferral된 **진짜 외부 선결조건**에만 적용. 누락·게으른 미검증엔 불가(그건 그냥 fail). §8.3 Web 권위는 불변 — Web가 deferral의 정당성 자체를 판정한다.
— 사례: T0-5 self 92, 행안부 위치정보요약DB 심사가 라이브 포맷·매칭 검증을 막음(조건부 통과 → `T0-5-live`에서 풀 통과).
